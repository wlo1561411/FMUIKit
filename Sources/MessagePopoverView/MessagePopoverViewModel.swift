...
extension MessagePopoverView.ViewModel {
    ...
    private func adjustSource(_ texts: [String]) -> [String?] {
        ...
        for text in texts.enumerated() {
            insertedSeparator.append(text.element)

            // 代表分隔線
            if text.offset < texts.count - 1 {
    ...
    func getText(at indexPath: IndexPath) -> String? {
        guard texts.count > indexPath.row, let text = texts[indexPath.row] else {
            return nil
        }
        return text
    }
