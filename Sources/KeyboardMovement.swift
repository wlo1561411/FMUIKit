        ...
        keyboardHeightPublisher
            .map { [weak self] height in
                ...
                let maxYminXOrigin = CGPoint(
                    x: origin.x,
                    y: origin.y + textfield.frame.height + keyboardAboveSpacing
                )

                let estimated = origin.y - keyboardFrame.origin.y + textfield.frame.height + keyboardAboveSpacing
...
@available(iOS 14.0, *)
extension KeyboardMovement where Self: UIViewController {
    public func observeKeyboardInteraction(cancellables: inout Set<AnyCancellable>) {
        view.addHighlightGesture(
