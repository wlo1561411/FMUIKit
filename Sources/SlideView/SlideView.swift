...
extension SlideView {
    private func prepareSwitch(to index: Int) {
        if index == preIndex || index < 0 || isSwitching {
            return
        }
        ...
        baseViewController?.transition(
            ...
            animations: { [weak self] in
                ...
                if self?.animatable ?? false {
                    preViewController.view.frame = endRect
                }
            },
    ...
    private func animateBack(with _: CGFloat) {
        UIView.animate(
            ...
            if toIndex >= 0, toIndex != preIndex, toIndex < viewControllersCount {
                preViewController?.beginAppearanceTransition(true, animated: false)
                removeToViewController()
                preViewController?.endAppearanceTransition()

                if let pre = preViewController {
                    delegate?.didCancelSwitch?(to: preIndex, viewController: pre)
                }
            }
        }
    }
    ...
    private func panEnded(_ offset: CGFloat) {
        ...
        completion: { [weak self] _ in
            ...
            if toIndex >= 0, toIndex < viewControllersCount {
                ...
                toViewController?.didMove(toParent: baseViewController)

                preIndex = toIndex
                preViewController = toViewController
                toViewController = nil
                toIndex = -1
            }

            if let pre = preViewController {
                delegate?.didSwitch?(to: preIndex, viewController: pre)
            }
        }
    }
}
    ...
    private func viewController(with index: Int) -> UIViewController? {
        if let cache {
            if let cachedObject = cache.object(for: "\(index)") {
                ...
                guard let object = dataSource?.viewController(self, at: index) else {
                    return nil
                }
